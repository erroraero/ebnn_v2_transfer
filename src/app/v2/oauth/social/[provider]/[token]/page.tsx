    ...
    return (
        <main className="min-h-screen bg-black text-white font-inter selection:bg-white/20 relative overflow-hidden flex flex-col items-center justify-center bg-mesh p-6">
            ...
            <div className="max-w-md w-full space-y-8 relative z-10">
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex justify-between items-center mb-8 px-2">
                ...
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass rounded-[2rem] p-8 md:p-10 border border-white/5 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="space-y-8">
                        <div className="space-y-4">
                            ...
                            <div className="space-y-1">
                                <h1 className="text-3xl font-black font-outfit uppercase tracking-tighter text-gradient-bw">Identity Entry</h1>
                        ...
                        <AnimatePresence>
                            {(error || isExpired) && (
                                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                                    ...
                                    {isExpired && (
                                        <button onClick={() => window.location.reload()} className="w-full py-2 text-[8px] font-black uppercase tracking-widest text-white/30 hover:text-white transition-colors">
                        ...
                        <div className="space-y-4">
                            <button onClick={handleLogin} disabled={loading || isExpired} className="w-full h-16 bg-white text-black rounded-2xl flex items-center justify-center gap-4 group/btn relative overflow-hidden transition-all active:scale-[0.98] disabled:opacity-30 disabled:grayscale hover:shadow-[0_0_20px_rgba(255,255,255,0.1)]">
                                {loading ? (
